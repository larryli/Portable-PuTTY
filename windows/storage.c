                    ...
                    const char *keytype, const char *key)
{
    ...
    hostkey_regname(regname, hostname, port, keytype);
        ...
        *otherstr = get_reg_sz(rkey, regname->s);
    ...
    compare = otherstr ? strcmp(otherstr, key) : -1;
        ...
        if (!otherstr) {
                ...
                p = snewn(256, char);
                userMB = MessageBox(NULL, "主机密钥已缓存在注册表。"
                        "确认要移动到文件配置中保存么？\n\n"
                        "是 \t-> 移动 (从注册表中删除)\n"
                        "否 \t-> 复制 (继续保存在注册表中)\n"
                ...
                if ((userMB == IDYES) || (userMB == IDNO)) {
                        ...
                        if (hFile == INVALID_HANDLE_VALUE) {
                                errorShow("无法创建文件 (不会从注册表删除密钥)", p);
                                userMB = IDNO;
                        }
                        else {
                                if (!WriteFile(hFile, key, strlen(key), &bytesRW, NULL)) {
                                        errorShow("无法保存密钥到文件 (不会从注册表删除密钥)", NULL);
                                        userMB = IDNO;
                                }
                                CloseHandle(hFile);
                        }
    ...
    (const char *add, const char *rem, char **out)
{
    ...
    *piterator_new = '\0';
    ...
    ++piterator_new;
        ...
        if (hFile == INVALID_HANDLE_VALUE) {
                errorShow("无法打开用来写入的文件", jumplistpath );
